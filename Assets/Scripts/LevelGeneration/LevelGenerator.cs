...
{
    public static event System.Action<LevelPathData> OnLevelGenerationComplete;
    ...
    void Start()
    {
        ...
        if (gridManager == null || pathfinder == null || tower == null || spawnPoints.Count == 0)
        {
            ...
            OnLevelGenerationComplete?.Invoke(null);
            return;
        }
    ...
    private IEnumerator GenerateLevelCoroutine()
    {
        ...
        for (int i = 0; i < maxRetries; i++)
        {
            ...
            if ((float)obstacleCount / candidateNodes.Count > 0.5f)
            {
                Debug.LogWarning("Obstacle coverage is over 50%. This may impact performance or pathability.");
            }

            yield return null;

            if (RecalculateAllRoutes())
            {
                Debug.Log("Initial path generation successful!");
                //DrawPaths();
                OnLevelGenerationComplete?.Invoke(FinalLevelPaths);
                yield break;
            }

            Debug.LogWarning("Path validation or route generation failed. Retrying with lower density.");
